...
function getAvatar(name) {
  let h = 0
  for (const c of name) h = (h + c.charCodeAt(0)) % emojis.length
  return emojis[h]
}
...
function App() {
  ...
  useEffect(() => { fetchPosts(); const id = setInterval(fetchPosts, 8000); return () => clearInterval(id) }, [])
        ...
        <div className={`drawer ${showForm ? 'open' : ''}`}>
          ...
          <form onSubmit={submit}>
            <div className="drawer-body">
              ...
              <input ref={fileRef} type="file" accept="image/jpeg,image/png,image/gif,image/webp" onChange={pickImage} hidden />
